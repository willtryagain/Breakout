...
class Dragon(Thing):
    ...
    def shift_up(self):
        temp = np.array([[' ' for j in range(self._head.shape[1])] for j in range(self._head.shape[0])], dtype='object')
    ...
    def show(self):
        ...
        if self._big_count % 3 == 0:
            if self._fine_count % 4 == 0 or self._fine_count % 4 == 3:
                self.shift_up()
            elif self._fine_count % 4 == 1 or self._fine_count % 4 == 2:
                self.shift_down()
            self._fine_count += 1
